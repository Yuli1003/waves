...
const WaveformAddition = ({ size, waveforms, convergence, isInverted }: Props) => {
  ...
  // Update waveform colors if inverted (but keep spectral/rainbow colors)
  const waveformsWithNewPoints = waveforms.map((waveform, index) => {
    // Check if this waveform has a specific color (like spectral colors)
    // Only override to white if the color is black (default)
    const shouldInvertColor = isInverted && waveform.color === '#000000';

    return {
      ...waveform,
      color: shouldInvertColor ? '#ffffff' : waveform.color,
