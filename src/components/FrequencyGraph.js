...
class FrequencyGraph extends PureComponent<Props, State> {
  ...
  renderBars() {
    const { baseFrequency, baseAmplitude, shape, type, xMax, showFormantLabels, voicePattern, isInverted } = this.props;
    ...
    return drawableHarmonics.map((harmonic, index) => {
      const { frequency, amplitude, label, color } = harmonic;
      ...
      return (
        <Fragment key={index}>
          <Bar
            ...
            x2={xCoordinate}
            y2={yCoordinate}
            color={FORMANT_COLORS[label]}
            onMouseEnter={() => this.toggleHover(index)}
            onMouseLeave={() => this.toggleHover(null)}
          />

          {showFormantLabels && label && (
            <FormantLabel x={xCoordinate} y={yCoordinate - 3} isInverted={isInverted}>
              {label}
            </FormantLabel>
          )}

          <FadeTransition typeName="g" duration={200} isVisible={isHovered}>
            <HoverText textAnchor="end" x={VIEWBOX_WIDTH} y={0} dy={5} isInverted={isInverted}>
...
const AxisLabel = styled.text`
  font-size: 3px;
  font-weight: regular;
  fill: ${props => props.isInverted ? '#ffffff' : '#000000'};
...
const FormantLabel = styled.text`
  font-family: 'Space Mono', monospace;
  font-size: 4px;
  font-weight: 700;
  fill: ${props => props.isInverted ? '#ffffff' : '#000'};
  text-anchor: middle;
  transition: fill 800ms ease-in-out;
`;
