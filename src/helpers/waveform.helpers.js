...
export const getWaveforms = ({
  ...
  switch (type) {
      ...
      return [
        {
          ...
          offset,
          strokeWidth: 3,
          color: secondaryWaveformColor || COLORS.secondary[500],
        },
        {
          shape: 'sine',
          frequency: baseFrequency,
    ...
    case 'spectral': {
      // Simplified visible light spectrum: red through violet
      // Frequencies increase from red (low) to violet (high)
      const spectralWaves = [
        {
          shape: 'sine',
    ...
    case 'voice': {
      // Voice pattern formant visualization
      // Uses voicePattern parameter to determine which vowel to display
      const selectedPattern = voicePattern || 'ah';
      const pattern = VOICE_PATTERNS[selectedPattern];

      if (!pattern) {
        throw new Error(`Unknown voice pattern: ${selectedPattern}`);
      }

      const voiceWaves = pattern.formants.map(formant => ({
        shape: 'sine',
        frequency: baseFrequency * formant.relativeFreq,
        amplitude: baseAmplitude * formant.amplitude,
        offset: 0,
        strokeWidth: 3,
        color: formant.color,
        label: formant.name, // For potential labeling
      }));
